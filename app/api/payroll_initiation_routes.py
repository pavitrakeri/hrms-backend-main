...
from app.features.payroll.initiation_service import initiate_payroll, update_payroll_details, get_payroll_setup
...
async def get_payroll_setup_endpoint(employee_id: str, user=Depends(get_current_user)):
    db_pool = get_db_pool()
    async with db_pool.acquire() as conn:
        return await get_payroll_setup(conn, user, employee_id)